...
class db_o{
 ...
 var $db_u ='maildb';//数据库用户
 var $db_p = 'maildb001';//数据库密码
 var $db_k ='mailserver';//要使用的库
	var $sql=NULL;//查询语句
	var $l_db;//连接的资源
	var $errorl;//输出
	var $errorv;//错误原因
	...
	function __construct(){//构建(一般成功返回0，错误返回非0 )
		$this->l_db = @mysql_pconnect($this->db_h,$this->db_u,$this->db_p);
		if(!$this->l_db){
	...
	function q(){//数据修改性query//先设置$this->sql属性
		...
		$q_result = mysql_query($this->sql);//实际语句
		$this->sql = NULL;//还原
		...
		$this->q_inid = mysql_insert_id();//id
